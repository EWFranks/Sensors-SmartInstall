...
#include "src\UnoWifi48Sensor\UnoWifi48Sensor.h"
...
// MQTT Variables
  const long interval = 500; // Interval for sending messages in milliseconds
...
// #define WATER_PIN A0
// #define VIBE_PIN A1
#define ULTRASONIC_INPUT A0
#define ULTRASONIC_OUTPUT A1
...
void MqttHandler() {
  ...
  // Runs every X seconds
    ...
    int ultrasonicDistance = ultrasonic(ULTRASONIC_INPUT, ULTRASONIC_OUTPUT);


  if(ultrasonicDistance > 0) { // Added ultrasonic code to the detriment of the rest of the sketch

    // Send readings
    StaticJsonDocument<1024> mqttDoc;
    JsonArray readings = mqttDoc.createNestedArray("readings");

    // int waterLevel = water(WATER_PIN);
    // addReading(readings, "water_level", String(waterLevel), "percentage");

    // bool vibe = vibration(VIBE_PIN);
    // addReading(readings, "vibration", String(vibe), "percentage");

    addReading(readings, "object_distance", String(ultrasonicDistance), "cm");
